...
class CSVService {
  static const String _top3BlocksPath = 'assets/ml_models/TPS_Model/top3_productive_blocks.csv';
  static const String _allBlocksStatsPath = 'assets/ml_models/TPS_Model/productive_blocks_stats.csv';
  static const String _tasksDatasetPath = 'assets/ml_models/TPS_Model/tempo_sage_tasks_dataset.csv';
  ...
  Future<List<ProductiveBlock>> loadAllBlocksStats() async {
    try {
      final String csvContent = await rootBundle.loadString(_allBlocksStatsPath);
      ...
      for (var row in csvTable) {
        if (firstRow) {
          ...
          for (int i = 0; i < row.length; i++) {
            ...
            if (columnName.contains('category')) categoryIndex = i;
            else if (columnName.contains('weekday')) weekdayIndex = i;
            else if (columnName.contains('hour')) hourIndex = i;
          ...
          if (categoryIndex < 0 || weekdayIndex < 0 || hourIndex < 0 || completedIndex < 0) {
        ...
        try {
          ...
          final completed = row[completedIndex].toString().toLowerCase() == 'true' || 
      ...
      for (final category in categoryStats.keys) {
        ...
        for (final blockKey in categoryStats[category]!.keys) {
          ...
          if (parts.length == 2) {
            ...
            final completionRate = categoryStats[category]![blockKey]! / 10.0; // Normalizar entre 0-1
        ...
        blocksByCategory[category] = ProductiveBlock.sortByCompletionRate(blocks);
      }
      
      _logger.i('Bloques productivos por categoría cargados: ${blocksByCategory.length} categorías');
  ...
  List<ProductiveBlock> _parseProductiveBlocks(String csvContent) {
    ...
    final startIndex = csvTable.first.length >= 3 && 
                       (csvTable.first[0] is String && 
                        csvTable.first[0].toString().contains('weekday')) ? 1 : 0;
  ...
  Future<void> saveProductiveBlocks(List<ProductiveBlock> blocks, String filename) async {
    final dir = await getApplicationDocumentsDirectory();
    final file = File('${dir.path}/$filename');
    
    final List<List<dynamic>> rows = [
      ['start_weekday', 'start_hour', 'completion_rate', 'category'], // Cabecera
      ...blocks.map((block) => [block.weekday, block.hour, block.completionRate, block.category ?? '']),
    ];
